"""
================================================================================
MULTI-MODAL DTI + fMRI GNN CLASSIFICATION WITH DTI-ONLY NODE SELECTION (REWEIGHTING)
================================================================================

What this script implements (as requested):
1) Two independent unimodal trainings:
   - Train a DTI GNN (structural connectivity).
   - Train an fMRI GNN (functional connectivity).

2) Node selection is computed ONLY from DTI (DTI-only selection), then
   IMPORTED into fMRI.
   - We DO NOT remove any nodes.
   - We ONLY REWEIGHT selected nodes (multiplicative weights on node features).

3) Final classification uses ONLY the common subjects (intersection of IDs)
   between DTI and fMRI:
   - Extract unimodal embeddings for common subjects.
   - Train a fusion MLP on concatenated embeddings.

4) Training + optimization style kept like the "pasted AGM.py-style" code:
   - AdamW optimizer (decoupled weight decay)
   - CrossEntropyLoss (no label smoothing)
   - ReduceLROnPlateau scheduler on val_loss
   - Gradient clipping (global norm)
   - EarlyStopping on best val_loss (patience-based, restores best weights)
   - StratifiedKFold(shuffle=True, random_state=0)

Notes / assumptions:
- This code assumes both modalities use the same atlas ordering (same region indexing).
  If DTI and fMRI have different numbers of nodes, we adapt/pad/truncate weights.
- fMRI input paths are placeholders: edit them to match your dataset.
- Connectivity matrices can be CSV (preferred) or NPY for either modality.

Dependencies:
- torch, numpy, pandas, scikit-learn
- torch-geometric (PyG): torch_geometric
...
class MultiModalGNNConfig:
    ...
    # - Mirrors the "PersonalizedClinicalEncoder" logic in the attached code (PubMedBERT CLS embedding).
...
# PubMed / literature prior over nodes (optional; applied ONLY to SELECTED nodes)
    ...
    # ---------- HYPERPARAMETER SWEEPS (recommended) ----------
    ...
    sweep_mode: str = "sequential"  # "sequential" (recommended) or "full"
    ...
    # Fusion training (also AGM-style)
...
class DemographicInfo:
    ...
    def generate_clinical_description(self, use_apoe4: bool = True, mode: str = "clinical_no_diagnosis") -> str:
        ...
        mode:
          ...
          - "clinical_with_diagnosis": includes diagnosis sentence (NOT recommended; may leak label)
        """
        parts: List[str] = []
...
class PubMedNodePriorLoader:
    ...
    Two ways to provide the prior:
      1) External file via `prior_path` (recommended):
         - .npy: 1D array
...
# ============================================================
# EARLY STOPPING (AGM.py style)
# ============================================================

class EarlyStopping:
    """
    Early stopping on **best validation loss** (recommended).
    ...
    def __call__(self, train_loss: float, validation_loss: float, model: Optional[nn.Module] = None, epoch: Optional[int] = None):
        ...
        if improved:
            ...
            if self.restore_best_weights and (model is not None):
                # NOTE: deepcopy is important; state_dict tensors are references.
...
class DTINodeSelector:
    ...
    Selection is computed ONLY on DTI (as requested).
    """
...
def run_hparam_sweep_for_task(
    ...
    Two modes:
      - sequential (recommended): first tune thresholds using baseline (topk, rf),
...
def main():
    ...
    # ---- optional PubMedBERT clinical embedding (from demographics text; like attached code) ----
...
# ============================================================================
# SELF-CONTAINED SINGLE-ABLATION RUNNER (AUTOGENERATED, CORRECTED)
# ============================================================================
